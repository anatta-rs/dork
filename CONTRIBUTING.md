...
Each parser format ships as its own crate `crates/dork-<format>/`:

1. `cargo new --lib crates/dork-<format>`
2. Add to `[workspace.dependencies]` if other crates need it
3. Implement the `dork_core::Parser` trait
4. Tests + ≥ 95% coverage
